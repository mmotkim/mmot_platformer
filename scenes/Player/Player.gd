...
@export var max_speed: float = 320 #560
@export var acceleration: float = 520 #2880
@export var turning_acceleration : float = 450	 #9600
@export var deceleration: float = 1200  #3200
# ------------------------------------------ #

# GRAVITY ----- #
@export var gravity_acceleration : float = 2600 #3840
@export var gravity_max : float = 1500 #1020
# ------------- #

# JUMP VARAIABLES ------------------- #
@export var jump_force : float = 980 #2000 
@export var jump_cut : float = 0.25	 #0.25
@export var jump_gravity_max : float = 500 #500
@export var jump_hang_treshold : float = 2.0 #2.0
@export var jump_hang_gravity_mult : float = 0.1 #0.1
# Timers
...
func x_movement(delta: float) -> void:
	...
	# Decelerate to max_speed if no input and above max_speed
	if x_dir == 0 or abs(velocity.x) > max_speed:
		var target_speed = x_dir * max_speed
		velocity.x = move_toward(velocity.x, target_speed, deceleration * delta)
	else:
		# Accelerate or maintain current speed within limits
		var accel_rate: float = acceleration if sign(velocity.x) == x_dir else turning_acceleration
		velocity.x += x_dir * accel_rate * delta
